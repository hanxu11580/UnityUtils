...
public class LinkImageText : Text, IPointerClickHandler
{
    ...
    protected virtual string GetOutputText(string outputText)
    {
        ...
        m_HrefInfos.Clear();
        var indexText = 0;
        var indexTextNotSymbol = 0;
        foreach (Match match in s_HrefRegex.Matches(outputText))
        {
            var beforeHyperText = outputText.Substring(indexText, match.Index - indexText);
            s_TextBuilder.Append(beforeHyperText);
            s_TextBuilder.Append("<color=blue>");  // 超链接颜色

            var temp = beforeHyperText.Replace("\n", "").Replace("\r", "").Replace("\t", "").Replace(" ", "");
            var beforeHyperTextLength = temp.Length + indexTextNotSymbol;
            indexTextNotSymbol += beforeHyperTextLength;

            var group = match.Groups[1]; //链接 匹配第一个小括号内容
            var hyperLinkName = match.Groups[2].Value;
            var hrefInfo = new HrefInfo
            {
                //startIndex = s_TextBuilder.Length * 4, // 超链接里的文本起始顶点索引
                startIndex = beforeHyperTextLength * 4,
                //endIndex = (s_TextBuilder.Length + match.Groups[2].Length - 1) * 4 + 3,
                endIndex = (beforeHyperTextLength + match.Groups[2].Length - 1) * 4 + 3,
                name = group.Value
            };
            ...
            indexTextNotSymbol += match.Groups[2].Value.Length;
        }
