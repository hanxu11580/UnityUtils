...
namespace Feif.UIFramework
{
    ...
    {
        private static readonly Dictionary<Type, UIBase> instances = new Dictionary<Type, UIBase>();
        ...
        [SerializeField] private Image panelWindowMask;
        ...
        public static Image PanelWindowMask { get; set; }
        ...
        private void Awake()
        {
            ...
            PanelWindowMask = panelWindowMask;
        ...
        public static Task Hide(Type type)
        {
            if (IsWindow(type))
            {
                if (instances.TryGetValue(type, out var instance))
                {
                    ...
                    // 将遮罩放到上一层面板上面
                    UIBase previousWindowUI = null;
                    foreach (var ins in instances) {
                        // 忽略 当前正在关闭的面板，且未激活的面板(未勾选AutoDestroy将不会被销毁)
                        if (ins.Value == instance || !ins.Value.isActiveAndEnabled) {
                            continue;
                        }
                        if (IsWindow(ins.Key)) {
                            previousWindowUI = ins.Value;
                        }
                    }
                    if(previousWindowUI != null) {
                        var hierarchyIndex = previousWindowUI.transform.GetSiblingIndex();
                        PanelWindowMask.transform.SetSiblingIndex(hierarchyIndex);
                    }
                    else {
                        PanelWindowMask.gameObject.SetActive(false);
                    }
        ...
        /// 创建UI GameObject
        /// </summary>
        public static Task<UIBase> Instantiate(GameObject prefab, Transform parent = null, UIData data = null)
        {
        ...
        private static async Task<UIBase> RequestInstance(Type type, UIData data)
        {
        ...
        private static void ReleaseInstance(Type type)
        {
            ...
            if (instances.TryGetValue(type, out var instance))
            {
                if (!instance.AutoDestroy) return;

                UIFrame.Destroy(instance.gameObject);
        ...
        private static async Task<UIBase> InstantiateAsync(GameObject prefab, Transform parent, UIData data)
        {
        ...
        /// <summary>
        /// 不创建，重新走一遍创建流程，不会调用onCreate
        /// </summary>
        /// <param name="uibase"></param>
        /// <param name="parent"></param>
        /// <param name="data"></param>
        /// <returns></returns>
        public static async Task<UIBase> UnCreateInstantiateAsync(UIBase uibase, Transform parent, UIData data) {
            uibase.transform.SetParent(parent);
            var uibases = uibase.transform.BreadthTraversal()
                .Where(item => item.GetComponent<UIBase>() != null)
                .Select(item => item.GetComponent<UIBase>())
                .ToArray();
            TrySetData(uibase, data);
        ...
        private static async Task ShowAsync(Type type, UIData data = null)
        {
            try
            {
                ...
                if (IsPanel(type))
                {
                    ...
                    var uibases = instance.BreadthTraversal().ToArray();
                ...
                if (IsWindow(type))
                {
                    ...
                    await DoRefresh(uibases);
                    instance.gameObject.SetActive(true);
                    // 先设置把Mask在最上面，然后让弹窗盖上去
                    PanelWindowMask.transform.SetAsLastSibling();
                    instance.transform.SetAsLastSibling();
                    PanelWindowMask.gameObject.SetActive(true);
                    DoBind(uibases);
                    DoShow(uibases);
                }
                timeout.Cancel();
